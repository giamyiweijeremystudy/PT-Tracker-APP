...
export default function BmiCalculator() {
  ...
  useEffect(() => { if (profile?.height_cm) setHeight(String(profile.height_cm)); }, [profile]);
  ...
  const saveResult = async () => {
    if (!bmi||!user) return;
    const h=parseFloat(height), w=parseFloat(weight), cat=getCategory(bmi);
    setSaving(true);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
              ...
              {height && parseFloat(height) > 0 && (() => {
                ...
                return (
                  <p className="text-xs text-muted-foreground text-center">
                    Healthy weight for {height}cm: <span className="font-medium text-foreground">{(18.5*h*h).toFixed(1)} – {(24.9*h*h).toFixed(1)} kg</span>
                  </p>
                );
              })()}
      ...
      {savedResults.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Calculator className="h-5 w-5 text-primary" /> BMI History</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
